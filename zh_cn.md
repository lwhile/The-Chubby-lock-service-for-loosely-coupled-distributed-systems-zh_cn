...
Chubby有两个主要的组件，一个是通讯通道RPC：一个服务器，另一个是客户端应用的链接库。参见图1，所有的Chubby客户端和服务器的通讯都被客户端库仲裁。第三个代理服务器组件是可选的，这个将在3.1节讨论。

一个Chubby cell包含一组服务器（通常是5个）来做副本集。以便于减少失败的可能性（例如，在不同的racks里）。副本集采用分布式一致性协议来选择主节点；这个主节点必须是多数副本集选举出来的并且保证那些副本集不会由于几秒的间隔known as the master lease再次选举不同的主节点。这个master lease定期被多数副本集选举出来的主节点更新