...
每一个Chubby文件和目录都可能做为读写锁；即任意一个客户端句柄都有在独占（写）模式下持有锁，或者有多个数量的客户端句柄在将锁保持在共享（读）模式下。就跟大多数程序认识的互斥一样，这里的锁也属于协同锁。所有只有在其他人尝试获得同一个锁的情况下会导致冲突；持有一个称为F的锁即不是访问文件F所必须的，也不是为了防止其他的客户端访问。我们拒绝强制锁，这会让被锁住的对象无法被那些没有持有锁的客户端访问：

- Chubby锁通常是保护资源被其他服务使用，而不仅仅将锁和文件关联起来。如果想以有意义的方式强制使用强制锁，将会要求我们对那些服务做出更多的修改。

- 我们不希望强制用户为了调试程序或是管理而需要访问文件时关闭应用。大多数人的计算机在一个复杂的系统内要使用这种方法访问是很困难的。管理软件只能简单得通过引导用户关闭或重启它的应用来关闭强制锁。
