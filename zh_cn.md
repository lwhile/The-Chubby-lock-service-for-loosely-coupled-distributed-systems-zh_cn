...
### 2.8 会话与心跳
Chubby的会话就是指Chubby客户端与Chubby单位的联系；每隔一段时间出现并且通过心跳来维护即定时握手。除非Chubby客户端通知主，否则由于seesion的有效性使得客户端的句柄，锁和缓存数据全部都依旧有效(provided its session remains valid).（然而session的协议维护可能需要客户端知道缓存失效也要保持会话，具体看下面论述）

客户端发起请求时一个新的会话首先与一个Chubby单位的主节点通讯.当结束时断开会话，或者seesion空闲的结束会话(如果超时返回或者几分钟后没有新的请求)
...
（先翻译这么多，豪豪你看看翻译的怎么样，我倒是觉得翻译得不怎么样，还需要你润色润色。。。。）